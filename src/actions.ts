...
export async function upsertProfile(formData: FormData) {
  ...
  await prisma.profile.upsert({
    ...
    update: { ...newUserInfo },
    create: { email: session.user.email, ...newUserInfo },
  });
...
export async function postEntry(formData: FormData) {
  ...
  if (!image || typeof image !== "string") throw new Error("Image is missing.");

  const cleanedImage = image.trim();
  if (!cleanedImage) throw new Error("Image cannot be empty.");
...
export async function togglePostLike(formData: FormData) {
  ...
  const postIdValue = formData.get("postId") as string;
  if (!postIdValue) throw new Error("Post ID is missing.");

  const post = await prisma.post.findUnique({ where: { id: postIdValue } });
  ...
  if (existingLike) {
    ...
    await prisma.$transaction([
      prisma.postLike.create({
        data: { postId: postIdValue, authorEmail: session.user.email },
      }),
      prisma.post.update({
        where: { id: postIdValue },
...
export async function postReply(formData: FormData) {
  ...
  if (typeof textValue !== "string") throw new Error("Reply text is missing.");
  if (typeof postIdValue !== "string" || !postIdValue)
    throw new Error("Post ID is missing.");
  ...
  if (!parentComment || parentComment.postId !== postIdValue) {
...
export async function likeComment(formData: FormData) {
  ...
  const commentIdValue = formData.get("commentId") as string;
  const postIdValue = formData.get("postId") as string;

  if (!commentIdValue || !postIdValue)
  ...
  if (existingLike) {
    ...
    await prisma.commentLike.create({
      data: { commentId: commentIdValue, authorEmail: session.user.email },
    });
  }
...
export async function toggleFollow(formData: FormData) {
  ...
  const targetProfileIdValue = formData.get("targetProfileId") as string;
  const targetUsernameValue = formData.get("targetUsername") as string;

  if (!targetProfileIdValue) throw new Error("Target profile ID is missing.");
...
export async function startConversation(formData: FormData) {
  ...
  if (!targetProfileIdValue || !isObjectId(targetProfileIdValue)) {
  ...
  if (!currentUserProfile) throw new Error("Current user profile not found.");
  if (currentUserProfile.id === targetProfileIdValue) redirect("/messages");
...
export async function sendMessage(formData: FormData) {
  ...
  const conversationIdValue = formData.get("conversationId") as string;
  const bodyValue = formData.get("body");

  if (!conversationIdValue || !isObjectId(conversationIdValue))
...
export async function deletePost(formData: FormData): Promise<void> {
  ...
  await prisma.$transaction([
    prisma.commentLike.deleteMany({
      where: { comment: { postId: postIdValue } },
    }),
...
export async function editComment(formData: FormData): Promise<void> {
  ...
  if (!commentIdValue || !postIdValue || typeof textValue !== "string")
    throw new Error("Required fields missing.");
  ...
  const comment = await prisma.comment.findUnique({
    ...
    select: { authorEmail: true, postId: true },
  });
...
export async function deleteComment(formData: FormData): Promise<void> {
  ...
  await prisma.$transaction([
    prisma.commentLike.deleteMany({ where: { commentId: commentIdValue } }),
    prisma.comment.deleteMany({ where: { parentCommentId: commentIdValue } }),
    prisma.comment.delete({ where: { id: commentIdValue } }),
  ]);
