...
function adminActivityLabel(kind: string, de: boolean) {
  const labels: Record<string, [string, string]> = {
    ...
    "user-delete": ["Account gelöscht", "Account deleted"], "post-delete": ["Beitrag gelöscht", "Post deleted"], "comment-delete": ["Kommentar gelöscht", "Comment deleted"], "team-message": ["VibeTeam-Nachricht gesendet", "VibeTeam message sent"],
  };
...
export default async function AdminPage() {
  ...
  return <main className="mx-auto w-full max-w-4xl pb-24 md:pb-8">
    ...
    <section className="mt-6 overflow-hidden rounded-3xl border border-orange-400/30 bg-white shadow-xl dark:bg-slate-900">
      <header className="bg-linear-to-r from-(--ig-orange) to-(--ig-red) px-6 py-7 text-white"><p className="text-xs font-bold tracking-[.18em]">VIBE ADMIN</p><h1 className="mt-2 text-3xl font-black">{de ? "Moderation" : "Moderation"}</h1><p className="mt-2 text-sm text-white/85">{de ? "Meldungen und interne Abstimmung für das Admin-Team." : "Reports and internal coordination for the admin team."}</p></header>
      <div className="space-y-8 p-5 sm:p-7">
        ...
        <VibeTeamMessageComposer recipients={manageableProfiles.map((item) => ({ id: item.id, username: item.username, name: item.name }))} de={de} />
        <AdminUserManagement users={manageableProfiles.map((item) => ({ ...item, isProtected: isProtectedAdmin(item.email) }))} de={de} canDeleteUsers={canDeleteUsers} />
