...
export default async function AdminPage() {
  ...
  const [reports, notes, profiles, auditEntries] = await Promise.all([
    ...
    prisma.profile.findMany({ where: { isSystem: false }, select: { id: true, email: true, name: true, username: true, isAdmin: true }, orderBy: { name: "asc" } }),
  ...
  return <main className="mx-auto w-full max-w-4xl pb-24 md:pb-8">
    ...
    <section className="mt-6 overflow-hidden rounded-3xl border border-orange-400/30 bg-white shadow-xl dark:bg-slate-900">
      ...
      <div className="space-y-8 p-5 sm:p-7">
        ...
        <VibeTeamMessageComposer recipients={profiles.map((item) => ({ id: item.id, username: item.username, name: item.name }))} de={de} />
