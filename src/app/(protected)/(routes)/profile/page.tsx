...
export default async function ProfilePage({ searchParams }: ProfilePageProps) {
  ...
  const avatarSize = profileHeaderLayout === "compact" ? "7.25rem" : profileHeaderLayout === "spotlight" ? "13rem" : "8rem";
  const avatarInnerSize = profileHeaderLayout === "compact" ? "6.75rem" : profileHeaderLayout === "spotlight" ? "12.5rem" : "7.5rem";
  const avatarImageSize = profileHeaderLayout === "compact" ? "6.25rem" : profileHeaderLayout === "spotlight" ? "12rem" : "7rem";
  ...
  return (
    <main className={`mx-auto w-full max-w-5xl ${profileHeaderLayout === "spotlight" ? "rounded-3xl border border-slate-200 bg-slate-50/60 px-4 py-5 shadow-sm dark:border-slate-700/80 dark:bg-slate-800/25 sm:px-7" : ""}`}>
      ...
      <div className={`${profileHeaderLayout === "compact" ? "mx-auto mt-6 flex w-full max-w-5xl flex-col items-start gap-4 rounded-2xl border border-slate-200 bg-slate-50/60 p-5 text-left sm:flex-row-reverse sm:items-start dark:border-slate-700/80 dark:bg-slate-800/30" : profileHeaderLayout === "spotlight" ? "mx-auto mt-6 max-w-5xl rounded-3xl border border-slate-200 bg-white/60 px-5 pb-10 pt-8 text-center shadow-xl shadow-slate-900/15 dark:border-slate-700/80 dark:bg-slate-900/25 dark:shadow-black/25" : "mx-auto mt-6 max-w-5xl rounded-3xl p-5 text-center"} ${headerBackgroundStyle ? profileHeaderLayout === "spotlight" ? "min-h-[40rem]" : profileHeaderLayout === "compact" ? "min-h-[20rem]" : "min-h-[23rem]" : ""} ${headerBackgroundStyle ? "text-white [&_*]:!text-[color:inherit]" : ""}`} style={headerBackgroundStyle ? { ...headerBackgroundStyle, color: headerTextColor } : undefined}>
      ...
      <section className={`${profileHeaderLayout === "spotlight" ? "mx-auto mt-1 max-w-xl rounded-2xl bg-slate-950/25 p-4 text-center shadow-lg backdrop-blur-sm" : profileHeaderLayout === "standard" && headerBackgroundStyle ? "mx-0 mt-4 max-w-xl rounded-2xl bg-slate-950/25 p-4 text-left shadow-lg backdrop-blur-sm" : "mx-0 text-left"} ${profileHeaderLayout === "compact" ? "mt-0 max-w-2xl" : profileHeaderLayout === "spotlight" || headerBackgroundStyle ? "" : "mt-5 max-w-3xl"}`}>
        ...
        <Link
          href="/support"
          className={`${profileHeaderLayout === "spotlight" ? "mx-auto" : "mx-0"} mt-4 inline-flex items-center gap-2 rounded-full border border-cyan-300/70 bg-cyan-50/70 px-4 py-2 text-sm font-bold text-cyan-800 no-underline transition hover:-translate-y-0.5 hover:border-cyan-400 hover:bg-cyan-100 dark:border-cyan-400/40 dark:bg-cyan-500/10 dark:text-cyan-200 dark:hover:bg-cyan-500/20`}
        >
          <CircleHelp size={16} aria-hidden="true" />
          {de ? "Hilfe & Support" : "Help & support"}
        </Link>
        {profile.isAdmin ? <div className={`mt-4 flex flex-col ${profileHeaderLayout === "spotlight" ? "items-center" : "items-start"}`}> <Link
            href="/admin"
        ...
        {showLinks && profileLinks.length > 0 && <div className="mt-4"><ProfileLinks links={profileLinks} language={de ? "de" : "en"} centered={profileHeaderLayout === "spotlight"} accent={profileAccent} /></div>}
        {showShoutouts && shoutouts.length > 0 && <ProfileShoutouts shoutouts={shoutouts} language={de ? "de" : "en"} centered={profileHeaderLayout === "spotlight"} />}
        <ProfileMilestones
          milestones={profile.milestoneBadges}
          hiddenMilestones={profile.hiddenMilestoneBadges}
          language={de ? "de" : "en"}
          centered={profileHeaderLayout === "spotlight"}
        />
      </section>
      </div>
