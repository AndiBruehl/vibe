...
export default async function SettingsPage() {
  ...
  const profile = await prisma.profile.upsert({
    ...
    include: { profileLinks: { orderBy: { position: "asc" } }, shoutouts: { include: { targetProfile: { select: { id: true, username: true, name: true, avatar: true } } }, orderBy: { position: "asc" } } },
  });
  return (
    <main>
      <section className="flex items-center justify-between">
        ...
        >
          <button
            type="submit"
            className="group mr-14 flex items-center gap-2 rounded-xl bg-linear-to-r from-red-500 to-pink-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-black/20 transition hover:scale-[1.03] hover:shadow-xl"
          >
            <LogOut
              size={16}
