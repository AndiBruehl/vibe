    ...
    <section aria-busy="true" aria-label="Loading content" className="mx-auto w-full max-w-5xl space-y-6 pb-24 md:pb-8">
      ...
      {[0, 1].map((index) => (
        <article key={index} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl dark:border-white/10 dark:bg-white/5">
          ...
          <div className="lg:grid lg:grid-cols-[minmax(0,1.08fr)_minmax(22rem,0.92fr)]"><Bar className="aspect-square w-full rounded-none" /><div className="space-y-4 p-5"><Bar className="h-4 w-full" /><Bar className="h-4 w-5/6" /><Bar className="h-4 w-2/3" /><div className="pt-4"><Bar className="h-11 w-full" /></div></div></div>
        </article>
      ))}
