...
import { normalizeProfileAccent, normalizeProfileHeaderBackgroundImage, normalizeProfileHeaderBackgroundMode } from "@/profile-personalization";
...
export async function POST(request: Request) {
  ...
  await prisma.profile.upsert({ where: { email: session.user.email }, update: { profileHeaderBackgroundMode, profileHeaderBackgroundImage, profileHeaderBackgroundColor, profileHeaderBackgroundEnd }, create: { email: session.user.email, profileHeaderBackgroundMode, profileHeaderBackgroundImage, profileHeaderBackgroundColor, profileHeaderBackgroundEnd } });
  revalidatePath("/profile"); revalidatePath("/settings"); revalidatePath("/profile/[username]", "page");
  return NextResponse.json({ profileHeaderBackgroundMode, profileHeaderBackgroundImage, profileHeaderBackgroundColor, profileHeaderBackgroundEnd });
}
