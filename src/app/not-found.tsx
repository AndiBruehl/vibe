    ...
    <main className="min-h-screen bg-[#101828] px-4 py-5 text-slate-100 sm:px-8 sm:py-8">
      ...
      <section className="mx-auto grid min-h-[calc(100vh-7rem)] w-full max-w-2xl place-items-center py-8 text-center">
        <div className="w-full overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/80 p-7 shadow-2xl shadow-black/30 backdrop-blur sm:p-10">
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight text-white sm:text-4xl"><LocalizedText en="Oops, this corner of VIBE does not exist." de="Ups, diese Ecke von VIBE gibt es nicht." /></h1>
          <p className="mx-auto mt-4 max-w-lg text-sm leading-6 text-slate-300 sm:text-base"><LocalizedText en="Maybe the link moved, maybe it was never meant for you. No stress — we will get you back into the Vibe." de="Vielleicht wurde der Link verschoben, vielleicht war er nie für dich bestimmt. Kein Stress — wir bringen dich wieder zurück in den Vibe." /></p>
          ...
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href="/home" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-linear-to-r from-(--ig-orange) to-(--ig-red) px-5 py-2.5 text-sm font-bold text-white no-underline shadow-lg shadow-orange-950/30 transition hover:brightness-110">
              ...
              <LocalizedText en="Back to Home" de="Zur Startseite" />
            </Link>
            <Link href="/support" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-cyan-300/50 bg-cyan-400/10 px-5 py-2.5 text-sm font-bold text-cyan-100 no-underline transition hover:border-cyan-200 hover:bg-cyan-400/20">
              ...
              <LocalizedText en="Report to Support" de="Fehler an Support melden" />
            </Link>
          </div>

          <p className="mt-6 text-xs text-slate-400"><LocalizedText en="If a VIBE link brought you here, the support team would appreciate a short note." de="Wenn du über einen VIBE-Link hier gelandet bist, freut sich das Support-Team über einen kurzen Hinweis." /></p>
        </div>
      </section>
    </main>
  );
