...
export default function AdminUserManagement({ users, de, canDeleteUsers }: { users: AdminUser[]; de: boolean; canDeleteUsers: boolean }) {
  ...
  const visibleUsers = useMemo(() => users.filter((user) => !normalized || [user.name, user.username, user.email].some((value) => value?.toLocaleLowerCase().includes(normalized))), [users, normalized]);
  ...
  return <details className="group border-t border-slate-200 pt-6 dark:border-slate-700">
    ...
    <div className="pt-4">
      ...
      <label className="relative mt-4 block"><span className="sr-only">{de ? "Nutzer suchen" : "Search users"}</span><Search size={18} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={query} onChange={(event) => setQuery(event.target.value)} type="search" maxLength={120} placeholder={de ? "Name, Benutzername oder E-Mail suchen" : "Search name, username, or email"} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white py-2 pl-10 pr-3 text-sm text-slate-900 dark:border-slate-600 dark:bg-slate-950 dark:text-white"/></label>
