...
export default async function ArchivedPosts({ email, language }: { email: string; language: "en" | "de" }) {
  ...
  if (!posts.length) return <div className="rounded-2xl bg-white p-8 text-center shadow-md shadow-slate-200 dark:bg-slate-800 dark:shadow-slate-950"><p className="text-slate-600 dark:text-slate-300">{de ? "Dein Archiv ist leer." : "Your archive is empty."}</p><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{de ? "Archivierte Beiträge sind nur für dich sichtbar." : "Archived posts are visible only to you."}</p></div>;
