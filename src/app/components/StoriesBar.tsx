...
export default function StoriesBar({ stories: suppliedStories, viewerEmail }: { stories: Story[]; viewerEmail: string }) {
  ...
  const postPressStartedAt = useRef(0);
  ...
  useEffect(() => { setMuted(true); }, [openStoryIndex, slide]);
  ...
  async function upload(files: File[]) {
    ...
    if (files.some((file) => ((!file.type.startsWith("image/") && !["video/mp4", "video/webm", "video/quicktime"].includes(file.type)) || file.size > (file.type.startsWith("video/") ? 100 : 25) * 1024 * 1024))) { setError(de ? "Nutze Bilder bis 25 MB oder MP4/WebM/MOV-Videos bis 100 MB." : "Use images up to 25 MB or MP4/WebM/MOV videos up to 100 MB."); return; }
  ...
  return <>
    <section className="-mx-4 px-4 md:mx-0 md:px-0">
      <div className="overflow-x-auto pb-2"><div className="flex min-w-max gap-4 md:gap-5">
        <button type="button" onClick={() => setCreating(true)} className="group flex w-20 shrink-0 flex-col items-center" aria-label={de ? "Story erstellen" : "Create story"}>
