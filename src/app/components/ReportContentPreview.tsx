  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[110] flex flex-col bg-slate-950/70 p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={de ? "Gemeldeten Inhalt prüfen" : "Review reported content"}>
      <section className="mx-auto flex h-full w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-slate-700 bg-white shadow-2xl dark:bg-slate-950">
        ...
        <iframe src={previewHref} title={de ? "Gemeldeter Inhalt" : "Reported content"} onLoad={(event) => { const document = event.currentTarget.contentDocument; if (!document) return; const hideNavigation = () => { document.body.classList.add("vibe-admin-preview"); document.querySelectorAll<HTMLElement>("[data-vibe-desktop-nav], [data-vibe-mobile-nav]").forEach((element) => element.style.setProperty("display", "none", "important")); document.querySelector<HTMLElement>("[data-vibe-protected-content]")?.style.setProperty("padding-left", "0", "important"); document.querySelector<HTMLElement>("[data-vibe-protected-main]")?.style.setProperty("padding", "0", "important"); }; hideNavigation(); window.setTimeout(hideNavigation, 250); }} className="min-h-0 w-full flex-1 border-0 bg-white" />
      </section>
    </div>}
  </>;
}
