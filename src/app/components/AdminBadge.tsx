...
export default function AdminBadge({ isAdmin, isVerified, badges = [], hiddenBadges = [], showCurated = false, centered = false }: { isAdmin?: boolean | null; isVerified?: boolean | null; badges?: string[] | null; hiddenBadges?: string[] | null; showCurated?: boolean; centered?: boolean }) {
  ...
  return <span className={`inline-flex max-w-full shrink-0 flex-wrap items-center gap-1 ${centered ? "w-full justify-center text-center" : ""}`} aria-label={[isAdmin && "Administrator", isVerified && "Verified", ...visibleBadges.map((badge) => badge.label)].filter(Boolean).join(", ")}>
    ...
    {(visibleBadges.length || customBadges.length) ? <span className={`basis-full flex max-w-full flex-wrap items-center gap-1.5 pt-0.5 ${centered ? "w-full justify-center" : ""}`}>{visibleBadges.map((badge) => <span key={badge.key} className="relative inline-flex"><button type="button" title={badge.label} aria-label={badge.label} aria-expanded={expanded === badge.key} onClick={() => setExpanded((current) => current === badge.key ? null : badge.key)} className="inline-flex size-7 items-center justify-center rounded-full border border-violet-400/60 bg-violet-50 text-violet-700 transition hover:scale-105 hover:bg-violet-100 dark:bg-violet-500/15 dark:text-violet-200 dark:hover:bg-violet-500/25">{icon(badge.key)}</button>{expanded === badge.key ? <span role="status" className="absolute left-1/2 top-full z-20 mt-1 -translate-x-1/2 whitespace-nowrap rounded-lg border border-violet-300/60 bg-white px-2 py-1 text-[10px] font-black text-violet-800 shadow-lg dark:bg-slate-900 dark:text-violet-100">{badge.label}</span> : null}</span>)}{customBadges.map((badge) => { const key = `custom-${badge.id}`; return <span key={badge.id} className="relative inline-flex"><button type="button" title={badge.label} aria-label={badge.label} aria-expanded={expanded === key} onClick={() => setExpanded((current) => current === key ? null : key)} className="vibe-custom-profile-badge inline-flex size-7 items-center justify-center rounded-full border border-fuchsia-400/60 bg-fuchsia-50 p-0 text-sm dark:bg-fuchsia-500/15">{badge.icon}</button>{expanded === key ? <span role="status" className="absolute left-1/2 top-full z-20 mt-1 -translate-x-1/2 whitespace-nowrap rounded-lg border border-fuchsia-300/60 bg-white px-2 py-1 text-[10px] font-black text-fuchsia-800 shadow-lg dark:bg-slate-900 dark:text-fuchsia-100">{badge.label}</span> : null}</span>; })}</span> : null}
  </span>;
}
