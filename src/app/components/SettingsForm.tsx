...
export default function SettingsForm({ profile }: SettingsFormProps) {
  ...
  return (
    <form action={saveProfile} onChange={(event) => { if (event.target instanceof HTMLInputElement && event.target.type === "color") return; setIsDirty(true); }} className="space-y-5">
      ...
      <nav className="grid grid-cols-3 gap-1 rounded-xl bg-slate-100 p-1 dark:bg-slate-800" aria-label={copy("Settings sections", "Einstellungsbereiche")}>
        <button type="button" onClick={() => openSettingsSection("profile")} aria-label={copy("Profile", "Profil")} title={copy("Profile", "Profil")} className={`flex min-w-0 items-center justify-center gap-1.5 rounded-lg px-1.5 py-2 text-xs font-semibold whitespace-nowrap transition sm:gap-2 sm:px-3 sm:text-sm ${activeTab === "profile" ? "bg-white text-orange-600 shadow-sm dark:bg-slate-700 dark:text-orange-300" : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100"}`}><UserRound size={16} className="shrink-0" /><span>{copy("Profile", "Profil")}</span></button>
        <button type="button" onClick={() => openSettingsSection("preferences")} aria-label={copy("Appearance & privacy", "Darstellung & Privatsphäre")} title={copy("Appearance & privacy", "Darstellung & Privatsphäre")} className={`flex min-w-0 items-center justify-center gap-1.5 rounded-lg px-1.5 py-2 text-xs font-semibold whitespace-nowrap transition sm:gap-2 sm:px-3 sm:text-sm ${activeTab === "preferences" ? "bg-white text-orange-600 shadow-sm dark:bg-slate-700 dark:text-orange-300" : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100"}`}><SlidersHorizontal size={16} className="shrink-0" /><span className="sm:hidden">{copy("Design", "Design")}</span><span className="hidden sm:inline">{copy("Appearance & privacy", "Darstellung & Privatsphäre")}</span></button>
        <button type="button" onClick={() => openSettingsSection("account")} aria-label={copy("Account & app", "Konto & App")} title={copy("Account & app", "Konto & App")} className={`flex min-w-0 items-center justify-center gap-1.5 rounded-lg px-1.5 py-2 text-xs font-semibold whitespace-nowrap transition sm:gap-2 sm:px-3 sm:text-sm ${activeTab === "account" ? "bg-white text-orange-600 shadow-sm dark:bg-slate-700 dark:text-orange-300" : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100"}`}><ShieldCheck size={16} className="shrink-0" /><span className="sm:hidden">{copy("Account", "Konto")}</span><span className="hidden sm:inline">{copy("Account & app", "Konto & App")}</span></button>
      </nav>
