...
export default function ProfileTagPicker({ initial = [] }: { initial?: TaggedProfile[] }) {
  ...
  return <div className="space-y-2">
    <label className="block text-sm font-medium">{de ? "Personen markieren" : "Tag people"}</label>
    <div className="flex flex-wrap gap-2">
      {selected.map((profile) => <span key={profile.id} className="inline-flex items-center gap-2 rounded-full bg-orange-100 px-3 py-1 text-sm text-orange-900 dark:bg-orange-500/15 dark:text-orange-200">
        @{profile.username || profile.name || (de ? "Profil" : "profile")}
        <button type="button" aria-label={`${de ? "Entfernen" : "Remove"} ${profile.username || profile.name || (de ? "Profil" : "profile")}`} onClick={() => setSelected((current) => current.filter((item) => item.id !== profile.id))}>×</button>
      </span>)}
    </div>
